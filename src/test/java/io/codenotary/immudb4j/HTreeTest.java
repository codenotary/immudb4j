...
public class HTreeTest {
  ...
  public void t3() {
    ...
    try {
      tree.buildWith(null);
      Assert.fail("Tree buidWith(null) did not throw an exception.");
    } catch (IllegalArgumentException e) {
      // all good if it got here.
    } catch (MaxWidthExceededException | NoSuchAlgorithmException e) {
      Assert.fail("Tree buidWith(null) threw a wrong exception.");
    }

    try {
      tree.buildWith(new byte[maxWidth + 1][32]);
      Assert.fail("Tree buidWith(maxWidth+1) must throw an exception.");
    } catch (IllegalArgumentException | NoSuchAlgorithmException e) {
      Assert.fail("Tree buidWith(maxWidth+1) threw a wrong exception.");
